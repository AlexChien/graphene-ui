...
class WithdrawModal extends React.Component {
    ...
    constructor( props ) {
        ...
        let last_withdraw_address = localStorage.getItem(`history_address_last_${this.props.output_wallet_type}`) || '';

        // don't check unless last address is found
        if (this.isQualifiedAddress(last_withdraw_address)) {
            this.checkAddress( this.props.output_wallet_type, last_withdraw_address )
            .then( result => {
                if (this.state.withdraw_address === last_withdraw_address){
                    this.setWithdrawAddressResult(result);
                }
            })
            .catch( err => this.setWithdrawAddressResult(false) );
        }

        this.state = {
            withdraw_amount: null,
            withdraw_address: last_withdraw_address,
            withdraw_address_check_in_progress: this.isQualifiedAddress(last_withdraw_address),
            withdraw_address_is_valid: null,
            options_is_valid: false,
            confirmation_is_valid: false,
            withdraw_address_selected: last_withdraw_address,
            memo: '',
    ...
    setWithdrawAddressResult(result){
        this.setState({
    ...
    getValidateAddressUrl(wallet, address){
        return this.props.url.replace("{wallet}", wallet).replace("{address}", address);
    }

    isQualifiedAddress(address = ""){
        return address != "" && address.toString().length >= TransConf.withdraw_address_length_min;
    }

    checkAddress(wallet, address){
        let url = this.getValidateAddressUrl( wallet, address );
        ...
        .then( json => { return json.isValid })

        // do not always notify user
        // let application level decide when to catch
        // and notify user
        // .catch(err => {
    ...
    onSelectChanged(index, e) {
        ...
        let withdrawAddresses = JSON.parse(localStorage.getItem(`history_address_${this.props.output_wallet_type}`));
        let new_withdraw_address = withdrawAddresses[index];
        localStorage.setItem(`history_address_last_${this.props.output_wallet_type}`, new_withdraw_address);

        if (this.isQualifiedAddress(new_withdraw_address)) {
            this.checkAddress(this.props.output_wallet_type, new_withdraw_address)
            .then( result => {
                this.setWithdrawAddressResult(result);
            })
            .catch( err => { this.setWithdrawAddressResult(false);} );
        ...
        this.setState({
            ...
            withdraw_address_check_in_progress: this.isQualifiedAddress(new_withdraw_address),
            withdraw_address_is_valid: null
        });
    }
